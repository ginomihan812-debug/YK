...
local function startAutoInteract()
    ...
    Workspace.DescendantAdded:Connect(function(obj)
        task.wait(0.1)
        if autoInteractEnabled and hasInteractable(obj) then
            if not isNPC(obj) then
                local target = obj
                if obj:IsA("ClickDetector") or obj:IsA("ProximityPrompt") or obj:IsA("TouchInterest") then
                    target = obj.Parent
    ...
    autoInteractLoop = task.spawn(function()
        while true do
            if autoInteractEnabled and not isProcessing then
                local character = LocalPlayer.Character
                if character then
                    local hrp = character:FindFirstChild("HumanoidRootPart")
                    if hrp then
...
Tab3:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            interactableCache = {}
            StarterGui:SetCore("SendNotification", { Title = "功能提示", Text = "已关闭自动互动材料", Duration = 2, Icon = "rbxassetid://128981664025072" })
        end
    end,
})
