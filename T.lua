        ...
        if Value then
            if not speedConnection then
                speedConnection = RunService.Stepped:Connect(function()
                    if speedEnabled and speedVal > 0 and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            ...
            StarterGui:SetCore("SendNotification", { Title = "功能提示", Text = "已开启CFrame移速", Duration = 2, Icon = "rbxassetid://128981664025072" })
        else
            if speedConnection then
                speedConnection:Disconnect()
                speedConnection = nil
            end
            StarterGui:SetCore("SendNotification", { Title = "功能提示", Text = "已关闭CFrame移速", Duration = 2, Icon = "rbxassetid://128981664025072" })
        end
    end,
})
...
Tab2:CreateButton({
    ...
    Callback = function()
        ...
        if playerGui then
            ...
            StarterGui:SetCore("SendNotification", { Title = "功能提示", Text = "已摧毁银狼脚本UI", Duration = 2, Icon = "rbxassetid://128981664025072" })
        else
            StarterGui:SetCore("SendNotification", { Title = "错误提示", Text = "未找到PlayerGui", Duration = 2, Icon = "rbxassetid://128981664025072" })
        end
    end,
})
        ...
        if not isProcessing then
            ...
            if character then
                ...
                if hrp then
                    ...
                    if nearest then
                        teleportToInteractable(nearest)
                    end
                end
            end
        end
...
print("自动传送互动已加载（right armAstro已加入黑名单）")
        ]])()
    end)
end
