...
local TextLabel = Create("TextLabel",{Parent = ToolTip,BackgroundColor3 = Color3.new(1, 1, 1),BackgroundTransparency = 1,Position = UDim2.new(0, 2, 0, 2),Size = UDim2.new(0, 196, 0, 46),ZIndex = 3,Font = Enum.Font.SourceSans,Text = "这是一段较长的文本。",TextColor3 = Color3.new(1, 1, 1),TextSize = 14,TextWrapped = true,TextXAlignment = Enum.TextXAlignment.Left,TextYAlignment = Enum.TextYAlignment.Top})
...
function newRemote(type, data)
    ...
    local connect = Button.MouseButton1Click:Connect(function()
        ...
        if blocked then
            log.GenScript = "-- SimpleSpy 阻止了此远程事件发送到服务器\n\n" .. log.GenScript
        end
...
function genScript(remote, args)
    ...
    if #args > 0 then
        xpcall(function()
            ...
            gen ..= "-- 发生错误:\n--"..err.."\n-- TableToString 失败! 回退到旧版功能 (结果可能不准确)\nlocal args = {"
            xpcall(function()
                for i, v in next, args do
...
function formatstr(s, indentation)
    ...
    return '"' .. handled .. '"' .. (reachedMax and " --[[ 已达到最大字符串大小，更改 'getgenv().SimpleSpyMaxStringSize' 以调整最大值 ]]" or "")
end
...
if not getgenv().SimpleSpyExecuted then
    local succeeded,err = pcall(function()
        ...
        if not hookmetamethod then
            ErrorPrompt("由于您的执行器不支持 hookmetamethod，Simple Spy V3 将无法完全发挥功能。",true)
        end
...
newButton(
    "排除 (i)",
    function() return "点击排除此远程。\n排除远程将使 SimpleSpy 忽略它，但它仍然可以使用。" end,
    function()
        if selected then
...
newButton(
    "阻止 (i)",
    function() return "点击阻止此远程发送。\n阻止远程不会将其从 SimpleSpy 日志中移除，但不会继续发送到服务器。" end,
    function()
        if selected then
...
newButton(
    "禁用信息",
    function() return string.format("[%s] 切换函数信息 (因为在某些游戏中可能导致卡顿)", configs.funcEnabled and "已启用" or "已禁用") end,
    function()
        configs.funcEnabled = not configs.funcEnabled
        TextLabel.Text = string.format("[%s] 切换函数信息 (因为在某些游戏中可能导致卡顿)", configs.funcEnabled and "已启用" or "已禁用")
    end
)

newButton(
    "自动阻止",
    function() return string.format("[%s] [测试版] 智能检测并从日志中排除垃圾远程调用", configs.autoblock and "已启用" or "已禁用") end,
    function()
        configs.autoblock = not configs.autoblock
        TextLabel.Text = string.format("[%s] [测试版] 智能检测并从日志中排除垃圾远程调用", configs.autoblock and "已启用" or "已禁用")
        history = {}
        excluding = {}
    end
)

newButton("日志检查调用者",function()
